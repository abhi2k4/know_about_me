...
const About = () => {
  ...
  return (
    <section
      id="about"
      ref={sectionRef}
      className="relative w-full px-6 py-12 sm:px-12 md:py-20 bg-[#040404] border-b border-white/5 overflow-hidden"
    >
      <div className="w-full flex flex-col lg:flex-row justify-between items-start gap-12 lg:gap-16">
        ...
        <div className="flex-1 w-full flex items-center justify-center lg:justify-end lg:p-8 lg:pr-20">
          <motion.div 
            className="relative flex items-center justify-center h-48 sm:h-64 md:h-72 w-full max-w-[440px]"
            onMouseLeave={() => {
              if (!isMobile) {
          ...
          >
            {memories.map((photo, i) => {
              ...
              const xOffset = offset * (isMobile ? 50 : 75);
              ...
              return (
                <motion.div
                  key={i}
                  className="absolute w-[120px] sm:w-[160px] md:w-[190px] aspect-[4/3] rounded-2xl overflow-hidden border-2 border-white/10 shadow-2xl bg-[#0d0d0d] will-change-transform group cursor-pointer"
                  style={{ originY: 1 }}
                  animate={{
                    x: xOffset,
